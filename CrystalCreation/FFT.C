...
//********* Histograms ********//cern root
   ...
   c1_6->SetFrameFillColor(42);
    ...
    Double_t w;
    Double_t accel;
    ifstream file("vib_t.txt");
   ifstream file2("vib_x.txt");
   if(file.is_open()){
   if(file2.is_open()){
   	double tArray[n];
   ...
   TF1 *fsin = new TF1("fsin", "(sin((1.3)*x+3.7)", 0, 8);
   fsin->Draw();
   
  
   TH1D *hsin = new TH1D("hsin", "hsin", n+1, 0, 120);
   ...
   for (Int_t i=0; i<=n; i++){
    ...
    /* Double_t x = *(double *)tr->GetV1();
     Double_t y = *(double *)tr->GetV2();
     Double_t z = *(double *)tr->GetV3();
     Double_t t = *(double *)tr->GetV4();//*/
     w = fsin->Eval((xArray[i])/9.8);
     accel = -w;
      hsin->SetBinContent(i+1, accel);
   }
   hsin->Draw("same");
